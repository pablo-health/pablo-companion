...
/// REST client for practice mode endpoints. Uses native URLSession.
@MainActor
public final class PracticeAPIClient {
    ...
    public func fetchTopics() async throws -> [PracticeTopic] {
    ...
    public func createSession(topicId: String) async throws -> PracticeSessionResponse {
    ...
    /// Fetches the session record (status, duration, etc.) after it ends.
    public func getSessionDetail(sessionId: String) async throws -> PracticeSessionDetail {
        let (data, _) = try await makeRequest("/api/practice/sessions/\(sessionId)")
        return try JSONDecoder().decode(PracticeSessionDetail.self, from: data)
    }
    ...
    public func webSocketURL(ticket: String) -> URL? {
        let wsBase = baseURL
    ...
    public func fetchTicket() async throws -> String {
